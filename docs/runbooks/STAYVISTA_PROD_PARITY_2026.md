...
6. `clarify_required_ratio` > 0.60 또는 `avg_missing_slots` > 1.8이면 슬롯 추출 회귀/컨텍스트 누락 점검
7. `clarify_action_click_ratio` < 0.35 또는 단일 slot 비중 > 75%이면 액션 UX/slot 키워드 가중치 튜닝
8. clarify 클릭 이후 품질 비교:
